...
export default function Home() {
  ...
  const fetchRooms = () => {
    setLoading(true);
    fetch("https://rummy777-e2631948f8d9.herokuapp.com/api/room/public")
      .then((res) => res.json())
      .then((data) => {
        if (!data.data) {
        } else {
  ...
  return (
    <>
      ...
      {isAlert && (
        <Alert isOpen={isAlert} onClose={() => setIsAlert(!isAlert)} />
      )}
      ...
      <section className="mobile_view">
        <div className="header">
          ...
          <div className="header_container">
            {!profile ? (
              <LoginButton setOpenModal={setOpenModal} profile={profile} />
            ) : (
              <UserInfo
                profile={profile}
                setProfile={(p) => {
